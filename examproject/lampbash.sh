...
###################################################################
    ...
    php-mysql php-gd php-mbstring php-tokennizer php-json php-bcmath
    php-curl php-zip unzip -y  < /dev/null
...
###################################################################
 ...
 ####################################################################
   ...
   #########################################################################
     ...
     # MYSQL USER AND PASSWORD CONFIGURATION
   ...
   #########################################################################
    ...
    #########################################################################
     ...
     ./lampbash.sh senaike senaike88


   ############################################################################

   # EXCUTE KEY GENERATE AND COMMAND FOR PHP

   ###########################################################################
   
   sudo sed -i 's/DB_DATABASE=laravel/DB_DATABASE=senaike/' /var/www/html/laravel/.env

   sudo sed -i 's/DB_USERNAME=root/DB_USERNAME=senaike/' /var/www/html/laravel/.env 
   
   sudo sed -i 's/DB_PASSWORD=laravel/DB_PASSWORD=senaike88/' /var/www/html/laravel/.env
